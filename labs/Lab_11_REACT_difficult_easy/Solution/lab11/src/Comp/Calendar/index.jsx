    ...
    return (
        <>
            <div className="selectDate">
                <select defaultValue={date.getMonth()} onChange={setMonthSelect}>
                    {months.map((name, idx) =>
                        <option key={name} value={idx}>{name}</option>
                    )}
                </select>

                <select defaultValue={date.getFullYear()} onChange={setYearSelect} className="years">
                    {years.map((name, idx) =>
                        ...
                        {daysInMonth.map((wDays, idx) =>
                            <tr key={idx} className="week">
                                {wDays.map((date, idx) => typeof (date) !== 'number' ?
                                    <td
                                        ...
                                        style={{
                                            backgroundColor: date.getDate() === new Date().getDate() &&
                                                ...
                                                date.getMonth() === new Date().getMonth() ? 'skyblue' : 'white'
                                        }}>
                                        <Link to={`/about/${date.getDate()}/${date.getFullYear()}/${date.getMonth()}`} style={{ color: 'inherit', textDecoration: 'inherit' }} key={date.getDate()} className="dayContent">
                                            <div>
                                    ...
                                    </td> : <td key={idx} className="day" style={{ opacity: '0.5' }}>
